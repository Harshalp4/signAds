...
export function ContentCard({item}:{item:Content}){return <Link className={`content-card ${item.kind}`} href={hrefFor(item)}><div className="card-image"><img src={imageUrl(item.image)} alt={item.kind==='product'?productImageAlt(item):item.description} width="1000" height="700" loading="lazy"/><span className="round-arrow"><ArrowUpRight size={23}/></span></div><div className="card-meta"><span>{item.category}</span>{item.industry&&<span>{item.industry}</span>}</div><h3>{item.title}</h3><p>{item.description}</p></Link>}
